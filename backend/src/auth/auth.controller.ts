...
export class AuthController {
        ...
        @Post('change')
        async changePassword(@User() user: EmployeeAuth, @Body() changeInfo: any, @Query('type') field: any): Promise<boolean> {
                if (field && field === 'password') {

                        return this.authService.changePwd(user, changeInfo);
                }

                else if (field && field === 'email') {

                        return this.authService.changeEmail(user, changeInfo);
                }
        }
}
